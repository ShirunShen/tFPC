...
set.seed(2)
init = c(10000,0.0001,10000) #(1,1,5000)
...
#penalty = list(c(3090,0.0000809,13090),3)
lambmus = penalty[[1]][1]
lambmut = penalty[[1]][2]
lambpc  = penalty[[1]][3]
...
for(t in 1:n_forecast){
    abs_error =  abs(temp_forecast12 - temp_true12)
}

mean_abs_error_monthly = apply(abs_error, 2, function(x){return(mean(x,na.rm=TRUE))})
sd_abs_error_monthly = apply(abs_error,2,function(x){return(sd(x,na.rm=TRUE))})
