...
#		result=bary(v.se[t.se[i,1],],v.se[t.se[i,2],],v.se[t.se[i,3],],x.se,y.se)
#		lam1.se=result[,1];lam2.se=result[,2];lam3.se=result[,3];
#		I.se=which((lam1.se>=-tol) *(lam2.se>=-tol) * (lam3.se>=-tol)==1)
#		if(length(I.se)>0){
#		for(j in 1:nbasis.se){
#			c.se=rep(0,nbasis.se)
#			c.se[j]=1
...
################## beval
##################
#beval=function(v.be,t.be,d.be,x.be,y.be){
#	nbasis.be=(d.be+1)*(d.be+2)/2*nrow(t.be)
#	bmatrix.be=matrix(0,length(x.be),nbasis.be)
#	for(j in 1:nbasis.be){
#		c.be=rep(0,nbasis.be)
#		c.be[j]=1
#		bmatrix.be[,j]=seval(v.be,t.be,c.be,x.be,y.be)
#		}
#	return(bmatrix.be)
#	}
#
################### seval
################### evaluate polynomials with coefficients c.se at points (x.se,y.se)
#seval=function(v.se,t.se,c.se,x.se,y.se){
#	ntri.se=nrow(t.se)  # ntri is number of triangles
#	bmatrix.se=rep(0,length(x.se))
#	nbasis.se=length(c.se)/ntri.se # number of basis defined on each triangle
#	tol=100*.Machine$double.eps
#	for (j in 1:ntri.se){
#		result=bary(v.se[t.se[j,1],],v.se[t.se[j,2],],v.se[t.se[j,3],],x.se,y.se)
#		lam1.se=result[,1];lam2.se=result[,2];lam3.se=result[,3];
#		I.se=which((lam1.se>=-tol) *(lam2.se>=-tol) * (lam3.se>=-tol)==1)
#		#if (length(I.se)>0) bmatrix.se[I.se]=bmatrix.se[I.se]+loceval(lam1.se[I.se],lam2.se[I.se],lam3.se[I.se],c.se[((j-1)*nbasis.se+1):(j*nbasis.se)])
#		if (length(I.se)>0) bmatrix.se[I.se]=loceval(lam1.se[I.se],lam2.se[I.se],lam3.se[I.se],c.se[((j-1)*nbasis.se+1):(j*nbasis.se)])
#		}
#	return(bmatrix.se)
#	}
#
...
################## bary
################## based on formula 2.3 on page 18
