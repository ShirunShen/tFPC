...
tMAE_tFPCA = mean(abs(Texastemp38 - zhat38_tFPCA),na.rm=TRUE)
tMAE_mFPCA = mean(abs(Texastemp38 - zhat38_mFPCA_summary),na.rm=TRUE)



#write.table(rbind(MeanSE_tFPCA,MeanSE_mFPCA),"Prediction_realdata.txt")

pdf("test_Prediction_realdata.pdf",width=8,height=4)
plot(MAE_tFPCA, type="b",ylim=c(0.4,1.0),pch=19,xlab="Month",ylab="MAE",col="red",xaxt="n")
axis(1, at = c(1,3,5,7,9,11),label=c("Jan","Mar","May","Jul","Sep","Nov"))
lines(MAE_mFPCA, pch=18, type="b", col="blue",lty=2)
legend(10,1.0,legend=c("tFPC","mFPC"),col=c("red","blue"),lty=1:2, cex=0.9)
dev.off()
