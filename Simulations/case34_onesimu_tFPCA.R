...
onesimu <- function(seed,n,ni1,ni2,k1,k2,HJ1,HJ2,var_noise,casetype){
  ...
  splinebasis = create.fourier.basis(c(0,length(ni)),nbasis=1,period=12) #nbasis=3 is an issue. June 10, 2020
  basismatrix = eval.basis(1:n, splinebasis)
  ...
  #init = c(500,0.01)
  #penalty = simplex(K_fold=5,J,z,rawbasis,ni,Q2,Ene,init)
 
  #penalty = list(c(300,5),1)
  
  
  
  source("../functions/tFPC/CV_tFPCA.R")
  # test = c(1,0,100)
  # result = CrossVal(K_fold=5,z,point,edges,n,d,r,J,p,station,ni,basismatrix,Pt,test[1],test[2],test[3],"EMalgorithm_mu1")
  # result
  ...
  time2 = system.time({
    for(i in 1:nrow(grid_mupc)){
      result_penalty = c(result_penalty,CrossVal(K_fold=5,z,point,edges,n,d,r,J,p,station,ni,basismatrix,Pt,grid_mupc[i,1],0,grid_mupc[i,2],"EMalgorithm_mu1"))
    }
  })
  init = as.numeric(unlist(grid_mupc[which.min(result_penalty),]))
  
  #init = c(1000,1000)
  source("../functions/tFPC/tFPCA_Penaltyparameters_2pen.R")
  penalty = simplex(K_fold=5,z,point,edges,n,d,r,J,p,station,ni,basismatrix,Pt,init,"EMalgorithm_mu1")
  ...
  MAEalpha1 = min(mean(abs(alpha1 - alpha1hat)),mean(abs(alpha1+alpha1hat)))
  MAEalpha2 = min(mean(abs(alpha2 - alpha2hat)),mean(abs(alpha2+alpha2hat)))
  
  
  results = c(sigma_noise,sigma_eta1,sigma_eta2,angle_rho,TMIAE_mean,TMIAE_indiv,TMIAE_pcpart,parameters$iter, MAEalpha1, MAEalpha2, k1hat,k2hat, lambmus, lambmut, lambpc, as.numeric(seed))
...
#casetype="mu1withAR_tFPCA"
casetype="mu1withiid_tFPCA"
